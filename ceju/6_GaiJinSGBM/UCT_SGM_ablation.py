...
6) 自定义黄–蓝渐变配色（手臂 + 海星为暖色高视差、边缘带冷色过渡）。
"""
...
class StarBoostUCTSGM_V3:
    ...
    def compute_disparity(self, rectL, rectR):
        ...
        print(f"[StarBoost-V3] 预处理时间: {t_preprocess - t_start:.4f} 秒")
        ...
        t_cost = time.time()
        print(f"[StarBoost-V3] 代价体构建时间: {t_cost - t0_cost:.4f} 秒")

        print("[StarBoost-V3] texture-guided SGM aggregation ...")
        t0_sgm = time.time()
        agg_cost = self._sgm_aggregate(cost_vol, tex)
        t_sgm = time.time()
        print(f"[StarBoost-V3] SGM聚合时间: {t_sgm - t0_sgm:.4f} 秒")
        ...
        t_pp = time.time()
        print(f"[StarBoost-V3] 后处理时间: {t_pp - t0_pp:.4f} 秒")

        t_total = time.time() - t_start
        print(f"[StarBoost-V3] 总处理时间: {t_total:.4f} 秒")
...
def run_starboost_v3(left_path, right_path,
    ...
    cv2.imwrite(out_disp, disp_color)
    cv2.imwrite(out_roi, (roi * 255).astype(np.uint8))
    print(f"[StarBoost-V3] 视差图已保存: {out_disp}")
    print(f"[StarBoost-V3] ROI 掩膜已保存: {out_roi}")

    cv2.imshow("Left", imgL)
    cv2.imshow("Disp StarBoost-V3", disp_color)
...
if __name__ == "__main__":
    left_image_path = r"../0_image/100left/left_11.jpg"
    right_image_path = r"../0_image/100right/right_11.jpg"
    run_starboost_v3(left_image_path, right_image_path)
